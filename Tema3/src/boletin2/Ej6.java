...
public class Ej6 {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < apuesta.length; i++) {
			random = rand.nextInt(49) + 1; // Generar número aleatorio entre 1 y 49
			apuesta[i] = random;
		}
		...
		// Comparar los números de apuesta con los del array ganador utilizando
		// binarySearch
		for (int j = 0; j < apuesta.length; j++) {
			if (Arrays.binarySearch(ganador, apuesta[j]) >= 0) {
				aciertos++;
			}
		}
